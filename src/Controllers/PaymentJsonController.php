...
class PaymentJsonController extends Controller
{
    ...
    public function store(PaymentCreateRequest $request)
    {
        ...
        throw_if(((!$this->permissionService->is(auth()->id(), 'admin')) && (auth()->id() != $userPaymentMethod['user_id'])),
        ...
        $paymentData['type'] = ($request->has('subscription_id')) ? (PaymentService::RENEWAL_PAYMENT_TYPE) : (PaymentService::ORDER_PAYMENT_TYPE);
