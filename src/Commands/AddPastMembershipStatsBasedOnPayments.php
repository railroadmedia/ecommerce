...
class AddPastMembershipStatsBasedOnPayments extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'AddPastMembershipStatsBasedOnPayments {startDate?} {endDate?}';
     ...
     * @var string
     */
    protected $description = 'Create daily membership stats records for a past period based on payments';
    ...
    /**
     * AddPastMembershipStatsBasedOnPayments constructor.
     *
    ...
    public function handle()
    {
        ...
        $rows = $this->databaseManager->connection(config('ecommerce.database_connection_name'))
            ...
            ->where('ecommerce_subscriptions.brand', 'drumeo') // testing
            ->where('ecommerce_products.subscription_interval_type', 'month')
            ->where('ecommerce_products.subscription_interval_count', 1)
            ...
            ->whereBetween(
                ...
                [
                    $endDate->copy()->subMonth()->subDay()->toDateTimeString(),
                    $endDate->toDateTimeString(),
                ]
            )
            ->get();

        $this->info('Drumeo monthly: '.count($rows->pluck('user_id')->unique()));
        ...
        $rows = $this->databaseManager->connection(config('ecommerce.database_connection_name'))
            ...
            ->where('ecommerce_subscriptions.brand', 'drumeo') // testing
            ->where('ecommerce_products.subscription_interval_type', 'year')
            ->where('ecommerce_products.subscription_interval_count', 1)
            ->where('ecommerce_payments.total_paid', '>', 0)
            ->where('ecommerce_payments.total_refunded', 0)
            ->where('ecommerce_payments.status', 'paid')
            ->where(
                function (Builder $builder) use ($endDate) {
                    return $builder->whereNull('ecommerce_subscriptions.canceled_on')
                        ->orWhere('ecommerce_subscriptions.canceled_on', '>', $endDate->toDateTimeString());
                }
            )
            ->whereBetween(
                'ecommerce_payments.created_at',
                [
                    $endDate->copy()->subYear()->subDay()->toDateTimeString(),
                    $endDate->toDateTimeString(),
                ]
            )
            ->get();

        $this->info('Drumeo yearly: '.count($rows->pluck('user_id')->unique()));
