...
class PaymentJsonControllerTest extends EcommerceTestCase
{
    ...
    public function test_delete_payment_order_total()
    {
        $userId = $this->createAndLogInNewUser();
        $this->permissionServiceMock->method('canOrThrow')->willReturn(true);

        $currency = $this->getCurrency();
        $paymentOneDue = $this->faker->numberBetween(0, 1000);
        $paymentTwoDue = $this->faker->numberBetween(0, 1000);
        $conversionRate = $this->currencyService->getRate($currency);

        $initialTotal = $paymentOneDue * $conversionRate +
                        $paymentTwoDue * $conversionRate;

        $order = $this->fakeOrder([
            'total_paid' => $initialTotal
        ]);
        ...
        $this->assertDatabaseHas(
            ...
            [
                'id' => $order['id'],
                'total_paid' => $paymentTwoDue * $conversionRate
            ]
        );
    }
}
